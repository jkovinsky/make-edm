...
1. **Scrape** (`get_page.py`) — scrapes upcoming events from 19hz
2. **Parse** (`llm.py`) — sends events to Google Gemini (batch API) to extract artist names and dates as structured JSON
3. **Filter** — keeps only events happening the current week
4. **Search Spotify** (`search_artists.py`) — looks up each artist on Spotify, storing IDs and follower counts
5. **Rank** (`rank_artist.py`) — scores each artist 1–10 via Gemini (global superstar → underground local)
6. **Fetch tracks** (`get_tracks.py`) — pulls top tracks per artist, weighted by their score
